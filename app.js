...
const LS_KEY_WRONG     = 'psych_wrong_v1';

async function loadGlossary() {
  const cached = localStorage.getItem(LS_KEY_CACHE);
  if (cached) { try { GLOSSARY = JSON.parse(cached); } catch (e) {} }
  try {
...
function renderSyncBar() {
  ...
  if (!count) { syncBar.textContent = '용어를 불러오지 못했습니다. 인터넷 연결을 확인해 주세요.'; return; }
  const when = synced
    ? new Date(synced).toLocaleString('ko-KR', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })
    : '오프라인 캐시';
...
function getWrongCounts() {
  try { return JSON.parse(localStorage.getItem(LS_KEY_WRONG) || '{}'); } catch { return {}; }
}
function addWrongCount(en) {
  const counts = getWrongCounts();
  counts[en] = (counts[en] || 0) + 1;
  localStorage.setItem(LS_KEY_WRONG, JSON.stringify(counts));
}
...
// 이벤트 위임으로 북마크 클릭 처리 (inline onclick의 따옴표 충돌 방지)
document.getElementById('vocabList').addEventListener('click', e => {
  const btn = e.target.closest('[data-bm]');
  if (btn) toggleBookmark(btn.dataset.bm);
});

// ===== 단어장 =====
let vocabCategoryFilter = '전체';
let vocabSortMode       = 'default'; // 'default' | 'alpha' | 'abc'
...
function renderVocabSortChips() {
  ...
  if (!el) return;
  el.innerHTML = '';
  [['default', '기본'], ['alpha', '가나다순'], ['abc', 'ABC순']].forEach(([mode, label]) => {
    const chip = document.createElement('button');
...
function renderVocabList() {
  ...
  if (!filtered.length) { list.innerHTML = '<div class="empty-state">검색 결과가 없습니다.</div>'; return; }

  list.innerHTML = filtered.map(g => `
    <div class="vocab-card">
      <div class="en">
        ${escapeHtml(g.en)}${g.star ? ' <span class="star">★</span>' : ''}
        <button class="bookmark-btn${bm.has(g.en) ? ' bookmarked' : ''}" data-bm="${escapeHtml(g.en)}">🔖</button>
      </div>
...
// ===== 퀴즈 큐 생성 (자주 틀리는 문제 우선) =====
function buildQueue(n, freqWrong) {
  const total = Math.min(n, GLOSSARY.length);
  if (!freqWrong) return shuffle(GLOSSARY).slice(0, total);

  const counts   = getWrongCounts();
  const wrong    = GLOSSARY.filter(g => counts[g.en] > 0).sort((a, b) => (counts[b.en] || 0) - (counts[a.en] || 0));
  const noWrong  = shuffle(GLOSSARY.filter(g => !counts[g.en]));
  return [...wrong, ...noWrong].slice(0, total);
}
...
function beginMcqQuiz(n) {
  const freqWrong = document.getElementById('mcqFreqWrong').checked;
  showSubSection('mcq', 'mcqQuiz');
  mcqQueue = buildQueue(n, freqWrong);
...
document.getElementById('mcqNextBtn').addEventListener('click', () => {
  mcqIndex++;
  if (mcqIndex >= mcqQueue.length) showMcqResult(); else nextMcq();
});
...
function beginWrittenQuiz(n) {
  const freqWrong = document.getElementById('wrFreqWrong').checked;
  showSubSection('written', 'wrQuiz');
  wrQueue = buildQueue(n, freqWrong);
...
document.getElementById('wrSubmitBtn').addEventListener('click', () => {
  ...
  if (!wrRetrying) {
    ...
    finishWritten(scoreAnswer(combined, item.def) >= SCORE_CORRECT, item, combined);
  }
});
...
document.getElementById('wrNextBtn').addEventListener('click', () => {
  wrIndex++;
  if (wrIndex >= wrQueue.length) showWrittenResult(); else nextWritten();
});
...
const CHANGELOG = [
  ...
  },
  {
    date: '2026-06-30',
    items: [
      '단어장: 가나다순 / ABC순 정렬 옵션 추가',
      '단어장: 북마크 기능 추가 — 카드의 🔖 버튼으로 즐겨찾기 등록, 필터 칩 "북마크"에서 모아보기',
      '객관식 퀴즈: 국문 용어 고르기 → 용어 설명 고르기 형식으로 변경, 영문/국문 보고 설명 고르기 모드 선택 가능',
      '객관식·주관식 퀴즈: 자주 틀리는 문제 우선 출제 옵션 추가',
      '주관식 퀴즈: 핵심 키워드 기반 채점으로 완화',
      '업데이트 로그 메뉴 추가',
    ]
  }
];
